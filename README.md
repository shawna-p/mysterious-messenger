...
* As of 2024-04-18 (the time of this update) the version of Ren'Py used is 8.0.3. The code is also intended to be compatible on 7.5+, but it is recommended to use Ren'Py 8+ for best performance. Only the latest code on the master branch works on the latest engine version; the current release, v3.3.0, requires the line `define config.search_prefixes += [ "images/" ]` be added somewhere (e.g. in `options.rpy`) for 8.1 to work. For 8.2+ to work, the keyword argument `retain=False` must be added to the definition for `my_adv` in `game/01_mysme_engine/vn_mode.rpy`. 8.2 has not yet been extensively tested.
