...
For long chatrooms, it's a good idea to restrict how many messages appear on-screen at once to avoid taking up too much memory. You can do this with list splicing. The simplest version looks like ``for msg in chatlog[-20:]:`` to only show the 20 most recent messages, for example. Mysterious Messenger uses a variable for this value so the player can adjust it as well.